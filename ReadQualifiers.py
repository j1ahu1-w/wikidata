...
def insert_qualifiers(json_data):
            ...
            if 'qualifiers' in c:
                for qualifiers_key in c['qualifiers']:
                    ...
                    for q in Qualifiers_arrays:
                        ...
                        datatype = q['datatype']
                        if 'datavalue' in q:
                            datavalue_value= json.dumps(q['datavalue']['value']).replace('\'','').replace('}','').replace('{','')
                            datavalue_type= q['datavalue']['type']
                        else:
                        ...
                        print('---------------------------------------------------')'''
                        try:
                            sql = "insert into qualifiers(id, qhash, snaktype, datatype, datavalue_value, datavalue_type) values('%s','%s','%s','%s','%s','%s');" % (
                                id, qhash, snaktype, datatype, datavalue_value, datavalue_type)
                            cur.execute(sql)
                            con.commit()
    ...
    with open(path, 'r', encoding='utf-8') as file:
        for line in file:
            if line[0] == '[':
                ...
                insert_qualifiers(json_data)
    cur.close()
    con.close()
    print('Qualifiers 存储成功!')
